...
export class editEmployee {
    ...
    editFunc() {
        ...
        if (obj.flag == 0) {
            ...
            let sendVal = ((((document.getElementById("tbl1")! as HTMLTableElement).rows[rInd] as HTMLTableRowElement).cells[3] as HTMLTableCellElement).firstChild as HTMLInputElement).value;
            let sendVal2 = ((((document.getElementById("tbl1")! as HTMLTableElement).rows[rInd] as HTMLTableRowElement).cells[4] as HTMLTableCellElement).firstChild as HTMLInputElement).value;
            let sendVal0 = ((((document.getElementById("tbl1")! as HTMLTableElement).rows[rInd] as HTMLTableRowElement).cells[0] as HTMLTableCellElement).firstChild as HTMLInputElement).value;
            ...
            if (objValidation.InputValidation(sendVal0, rInd) && objValidation.ValidateEmail(sendVal, rInd) && objValidation.phonenumber(sendVal2, rInd)) {
                ...
                for (let i = 0; i < 8; i++) {
                    if (i === 5) {
                        sel = ((((document.getElementById("tbl1")! as HTMLTableElement).rows[rInd] as HTMLTableRowElement).cells[5] as HTMLTableCellElement).firstChild as HTMLSelectElement);
                        x[5].innerHTML = sel.options[sel.selectedIndex].text;
                        valRole = parseInt(sel.options[sel.selectedIndex].value);
                    }
                    else if (i === 7) {
                        sel = ((((document.getElementById("tbl1")! as HTMLTableElement).rows[rInd] as HTMLTableRowElement).cells[7] as HTMLTableCellElement).firstChild as HTMLSelectElement);
                        x[7].innerHTML = sel.options[sel.selectedIndex].text;
                        valCustomer = parseInt(sel.options[sel.selectedIndex].value);
                    }
                    else {
                        x[i].innerHTML = ((((document.getElementById("tbl1")! as HTMLTableElement).rows[rInd] as HTMLTableRowElement).cells[i] as HTMLTableCellElement).firstChild as HTMLInputElement).value;
                    }
                }
                let updatedUser = {
                    ...
                    c_id: valCustomer,
                    id: (document.getElementById('tbl1')! as HTMLTableElement).rows[rInd].id
                };
                let row = ((document.getElementById("tbl1")! as HTMLTableElement).rows[rInd] as HTMLTableRowElement);
                if (row.id == "new") {
