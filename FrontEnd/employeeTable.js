...
export class Operation {
    ...
    generate_table(Employee) {
        ...
        for (var i = 0; i < arrlen; i++) {
            ...
            for (let [key, value] of Object.entries(employeeArray)) {
                if (z == objLen - 2) {
                    ...
                    let cellText = document.createTextNode(Employee[i].role_name);
                    cell.appendChild(cellText);
                    row.appendChild(cell);
                }
                else if (`${key}` == "c_id") {
                    cell = document.createElement("td");
                    let cellText = document.createTextNode(Employee[i].name);
                    cell.appendChild(cellText);
                    row.appendChild(cell);
                }
                else {
            ...
            //Edit Button
            cell = document.createElement("td");
            let edit = objButton.createButton("Edit", "edit btn btn-success");
            obj.flag = 0;
            edit.onclick = obj.editFunc;
            cell.appendChild(edit);
            row.appendChild(cell);
            //delete button begins
            let del = objButton.createButton("Delete", "del btn btn-danger");
        ...
        let buttonMulti = objButton.createButton("Multiple Delete", "loadData btn btn-primary");
        buttonMulti.onclick = obj.MultipleDelete;
        buttonMulti.id = "multiDel";
        body.appendChild(buttonMulti);
        let addRow = objButton.createButton("Add Row", "add-row btn btn-primary");
    ...
    MultipleDelete() {
        let y = document.getElementById("tbl1").rows;
        console.log(y.length);
        for (let i = 0; i < (y.length) + 1; i++) {
            let x = document.getElementById("check" + i);
            console.log(x);
            if (x.checked == true) {
                let rInd = x.parentNode.parentNode.rowIndex;
                console.log(x);
    ...
    refreshFunc() {
        let copy = document.getElementById('tbl1');
        console.log(copy);
        copy.parentNode.removeChild(copy);
        let copy2 = document.getElementById("multiDel");
        copy2.parentNode.removeChild(copy2);
    }
    addRowFunc() {
